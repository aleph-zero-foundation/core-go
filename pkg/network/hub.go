...
type Hub interface {
	core.Service
	// Dial connects to a committee member identified by pid using provided band and returns the resulting connection or an error.
	Dial(pid uint16, band int) (Connection, error)
	// Listen for an incoming connection on the provided band. Returns the connection if successful, otherwise an error.
